...
public class HoaDonDTRepo {
    ...
    public List<HoaDonDoiTraViewModel> all() {
        List<HoaDonDoiTraViewModel> lisstHDDT = new ArrayList<>();
        try {
            Connection conn = JDBCUtil.getConnection();
            String sql = "select Id,MaDoiTra,NgayDoiTra,NgayNhanSP,SoLuong from HoaDonDoiTra";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.execute();
            ResultSet rs = ps.getResultSet();
            while (rs.next()) {
                String id = rs.getString("Id");
                String madt = rs.getString("MaDoiTra");
                Date ngayDt = rs.getDate("NgayDoiTra");
                Date ngayNhan = rs.getDate("NgayNhanSP");
                int sl = rs.getInt("SoLuong");
                HoaDonDoiTraViewModel hdv = new HoaDonDoiTraViewModel();
                hdv.setId(id);
                hdv.setMa(madt);
    ...
    public Integer insertHDDTR(HoaDonDoiTra hdt) {
        ...
        try {
            Connection conn = JDBCUtil.getConnection();
            String sql = "Insert into HoaDonDoiTra " + "(MaDoiTra,NgayDoiTra,NgayNhanSP,SoLuong)" + " Values(?,?,?,?)";
            PreparedStatement ps = conn.prepareStatement(sql);
    ...
    public Integer deleteHDDTR(String madt) {
        ...
        try {
            Connection conn = JDBCUtil.getConnection();
            String sql = "Delete from HoaDonDoiTra Where MaDoiTra=?";
            PreparedStatement ps = conn.prepareStatement(sql);
